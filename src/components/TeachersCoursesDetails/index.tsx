    ...
    <>
      <section className="mt-12">
        ...
        <div className="bg-white rounded-lg p-6 flex flex-col md:flex-row items-center">
          <Image
            src={getPathImage(tutors?.imagem?.path)}
            alt={tutors?.title}
            width={200}
            height={150}
          ...
          />
          <div>
            <h3 className="text-xl font-bold mb-2">{tutors?.title}</h3>
            <div className="flex flex-row gap-px">
              {tutors?.specialty?.map((specialty: any) => {
                return (
            ...
            <p className="text-gray-600 mt-2">{tutors?.description}</p>
          </div>
        </div>
      </section>
    </>
